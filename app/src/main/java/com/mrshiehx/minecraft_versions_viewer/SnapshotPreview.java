...
public class SnapshotPreview extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Utils.initialization(this, R.string.activity_snapshot_preview_name);
        ...
        // Special of snapshot preview
        TextView tooManyContentRestartTip = new TextView(this);
        tooManyContentRestartTip.setText(R.string.text_activity_snapshot_preview_textview_too_many_content_restart_tip);
        mainLayout.addView(tooManyContentRestartTip, 1);

        ListView listview = new ListView(this);
        mainLayout.addView(listview, 2);
        versionsAdapter = new VersionsAdapter(this, R.layout.version_item, versions);
        listview.setAdapter(versionsAdapter);

        initListview();
        tooManyContentRestartTip.setVisibility(View.GONE);
    }

    DialogInterface.OnClickListener goToSettings = new DialogInterface.OnClickListener() {
        @Override
        public void onClick(DialogInterface dialog, int which) {
            startActivity(new Intent().setClass(SnapshotPreview.this, Settings.class));
        }
    };
    ...
    public void initListview() {
        ...
        if (sharedPreferences.getString("snapshot_preview_versions_source", "default").equals("default")) {
            ...
            if (TextUtils.isEmpty(sharedPreferences.getString("input_snapshot_preview_versions_source", null)) == false) {
                if (Utils.isUrlAUrl(sharedPreferences.getString("input_snapshot_preview_versions_source", null)) == true) {
                    String fileNameOfInputerVersionsSourceGetText = sharedPreferences.getString("input_snapshot_preview_versions_source", null).substring(sharedPreferences.getString("input_snapshot_preview_versions_source", null).lastIndexOf("/") + 1);
                    ...
                    if (fileExtensionOfInputerVersionsSourceGetText.equals("json") == true) {
                        try {
                            Utils.downloadAndInitListview(
                                    this,
                                    sharedPreferences.getString("input_snapshot_preview_versions_source", null),
            ...
            if (TextUtils.isEmpty(sharedPreferences.getString("input_snapshot_preview_versions_source", null)) == false) {
                String localFileName = sharedPreferences.getString("input_snapshot_preview_versions_source", null).substring(sharedPreferences.getString("input_snapshot_preview_versions_source", null).lastIndexOf("/") + 1);
                if (Utils.fileIsExists(this, sharedPreferences.getString("input_snapshot_preview_versions_source", null)) == true) {
