...
public class Utils {
    ...
    public static void deleteFile(Context context, String filePathAndName) {
        try {
            ...
            if (f.exists()) {
                if (f.delete()) {
                    Log.i("MSVV.Utils.deleteFile", "Delete file (" + filePathAndName + ") is success!");
                } else {
                    showDialog(context, "", "delete failed");
                    Log.e("MSVV.Utils.deleteFile", "Delete file (" + filePathAndName + ") is failed!");
                }
            } else {
                Log.e("MSVV.Utils.deleteFile", "Delete file (" + filePathAndName + ") is not exists!");
            }
        } catch (Exception e) {
            Looper.prepare();
            Utils.exceptionDialog(context, e);
    ...
    public static void initListview(Context context, List<VersionItem> listviewItem, String needLoadedFilePathAndName, String needLoadedArrayName) throws JSONException {
        String strJsondata = Utils.getJson(context, needLoadedFilePathAndName);
                ...
                public void run() {
                    try {
                        ...
                        timer.schedule(new TimerTask() {
                            @Override
                            public void run() {
                                if (Utils.fileIsExists(context, Utils.getDataCachePath(context) + "/" + afterDownloadFileName) == true) {
                                    //Looper.prepare();
                                    Utils.deleteFile(context, Utils.getDataCachePath(context) + "/" + afterDownloadFileName);
                                    //Looper.loop();
                                    //Looper.prepare();
                                    //有效的，所以不显示
